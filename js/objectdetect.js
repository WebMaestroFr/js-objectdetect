...
(function(module) {
    ...
    var/**
        ...
        mirrorImage = function(src, srcWidth, srcHeight, dst) {
            if (!dst) 
                dst = new src.constructor(srcWidth * srcHeight);
        ...
        computeCanny = function(src, srcWidth, srcHeight, dst) {
            ...
            if (!dst) 
                dst = new src.constructor(srcLength);
            var buffer1 = dst === src
                ? new src.constructor(srcLength)
                : dst;
            ...
            for (var x = 2; x < srcWidth - 2; ++x) {
                var index = x;
                for (var y = 0; y < srcHeight; ++y) {
                    buffer1[index] = 0.1117 * src[index - 2] + 0.2365 * src[index - 1] + 0.3036 * src[index] + 0.2365 * src[index + 1] + 0.1117 * src[index + 2];
                    index += srcWidth;
                }
            }
            ...
            for (var x = 0; x < srcWidth; ++x) {
                ...
                for (var y = 2; y < srcHeight - 2; ++y) {
                    index += srcWidth;
                    buffer2[index] = 0.1117 * buffer1[index - srcWidth - srcWidth] + 0.2365 * buffer1[index - srcWidth] + 0.3036 * buffer1[index] + 0.2365 * buffer1[index + srcWidth] + 0.1117 * buffer1[index + srcWidth + srcWidth];
                }
            }
            ...
            for (var x = 2; x < srcWidth - 2; ++x) {
                ...
                for (var y = 2; y < srcHeight - 2; ++y) {
                    ...
                    dst[index] = abs(-buffer2[index - 1 - srcWidth] + buffer2[index + 1 - srcWidth] - 2 * buffer2[index - 1] + 2 * buffer2[index + 1] - buffer2[index - 1 + srcWidth] + buffer2[index + 1 + srcWidth]) + abs(buffer2[index - 1 - srcWidth] + 2 * buffer2[index - srcWidth] + buffer2[index + 1 - srcWidth] - buffer2[index - 1 + srcWidth] - 2 * buffer2[index + srcWidth] - buffer2[index + 1 + srcWidth]);
                }
            }
            return dst;
        },
            ...
            for (var x = 0; x + windowWidth < width; x += step) {
                ...
                for (var y = 0; y + windowHeight < height; y += step) {
                    ...
                    var mean = (sat[satIndex] - sat[satIndex1] - sat[satIndex2] + sat[satIndex3]),
                        ...
                        variance = (ssat[satIndex] - ssat[satIndex1] - ssat[satIndex2] + ssat[satIndex3]) * area - mean * mean,
                        ...
                        for (var j = 0, jEnd = classifierUint32[++i]; j < jEnd; ++j) {
                            ...
                            if (classifierUint32[++i]) {
                                ...
                                for (var kEnd = i + classifierUint32[++i]; i < kEnd;) {
                                    ...
                                    simpleClassifierSum += classifier[++i] * (rsat[f1] - rsat[f2] - rsat[f3] + rsat[f2 + f3 - f1]);
                                }
                            } else {
                                // Simple classifier is not tilted:
                                for (var kEnd = i + classifierUint32[++i]; i < kEnd;) {
                            ...
                            complexClassifierSum += classifier[i + (simpleClassifierSum > std
                                    ? 2
                                    : 1)];
                            i += 2;
                        }
        ...
        groupRectangles = function(rects, minNeighbors, confluence) {
            ...
            for (var i = 0; i < rectsLength; ++i) {
                ...
                for (var j = 0; j < i; ++j) {
                    ...
                    if (abs(rect1[0] - rect2[0]) <= delta && abs(rect1[1] - rect2[1]) <= delta && abs(rect1[0] + rect1[2] - rect2[0] - rect2[2]) <= delta && abs(rect1[1] + rect1[3] - rect2[1] - rect2[3]) <= delta) {
            ...
            for (var i = 0; i < numClasses; ++i) {
                ...
                for (var j = i + 1; j < numClasses; ++j) {
                    ...
                    if ((r1[0] >= r2[0] - dx && r1[1] >= r2[1] - dy && r1[0] + r1[2] <= r2[0] + r2[2] + dx && r1[1] + r1[3] <= r2[1] + r2[3] + dy) || (r2[0] >= r1[0] - dx && r2[1] >= r1[1] - dy && r2[0] + r2[2] <= r1[0] + r1[2] + dx && r2[1] + r2[3] <= r1[1] + r1[3] + dy)) {
                        break;
                    }
                }
